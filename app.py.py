...
if excel_file is not None:
    ...
    # Si está en GitHub, puedes usarlo como ejemplo
    df = pd.read_excel("SECTOR SEGUROS.xlsx")

df.columns = [c.strip() for c in df.columns]

# Parsear números colombianos
def parse_num_co(x):
    try:
...
with st.container():
    ...
    # ARIMA Forecast para el resto del año
    if len(df_sel) >= 6:
        ...
        meses_restantes = 12 - last_month if last_month else 12
...
with st.container():
    ...
    if len(df_sel) >= 6:
        ...
        modelo = ARIMA(ts, order=(1,1,1))
        ajuste = modelo.fit()
        fc = ajuste.get_forecast(steps=horizonte)
        forecast_index = pd.date_range(ts.index.max() + pd.offsets.MonthBegin(), periods=horizonte, freq='MS')
        fc_df = pd.DataFrame({
