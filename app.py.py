...
st.set_page_config(page_title="AseguraView · Ciudades & Ramos", layout="wide", page_icon=":bar_chart:")
...
def css_background_image(data_url: str, blur_px: int = 2, darken: float = 0.35) -> str:
    ...
    <style>
      ...
      .stApp::before {{
        ...
        transform: scale(1.03); /* evita bordes vacíos tras blur */
        z-index: -1;
      }}
    </style>
    """
...
def css_background_animated() -> str:
    """Gradiente animado tech (sin imagen)."""
    return """
    <style>
      .stApp {
        position: relative;
...
def file_to_data_url(file) -> str:
    """Convierte archivo binario de imagen a data URL base64."""
...
with st.sidebar:
    st.header("Fondo")
    bg_mode = st.radio("Modo de fondo:", ["Animado (gradiente)", "Imagen subida", "Imagen por URL"], index=0)
    bg_css = ""
...
st.markdown(
    ...
    <style>
      ...
      .stMetric {background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.10);
                 border-radius: 14px; padding: 8px;}
      .stDataFrame th, .stDataFrame td {color: #d8e2ff !important;}
      .stSidebar, .stSelectbox, .stSlider {color: #d8e2ff !important;}
    </style>
    """,
...
def match_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for target, pats in COL_PATTERNS.items():
        ...
        for p in pats:
            ...
            if found: break
        if not found:
            raise ValueError(f"No se encontró columna para '{target}'. Encabezados del archivo: {list(df.columns)}")
...
with st.sidebar:
    ...
    anio = st.selectbox("Año YTD/Cierre:", list(range(max_year, min_year-1, -1)))
